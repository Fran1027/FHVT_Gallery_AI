from PyQt6.QtWidgets import QGraphicsRectItem, QGraphicsEllipseItem, QGraphicsObject
...
class ColorMarker(QGraphicsEllipseItem):
    ...
    def __init__(self, color, x, y, size=30):
        ...
        self.outer_ring = QGraphicsEllipseItem(-size//2 - 1, -size//2 - 1, size + 2, size + 2, self)
...
class CropOverlayItem(QGraphicsObject):
    ...
    def paint(self, painter, option, widget=None):
        ...
        r = self.crop_rect
        points = [
            (r.left(), r.top()), (r.right(), r.top()), (r.left(), r.bottom()), (r.right(), r.bottom()), # Esquinas
            (r.center().x(), r.top()), (r.center().x(), r.bottom()), (r.left(), r.center().y()), (r.right(), r.center().y()) # Bordes
        ]
        for x, y in points:
    ...
    def _get_hotzone(self, pos):
        ...
        m = self.margin
        # Esquinas
        if QRectF(r.left()-m, r.top()-m, m*2, m*2).contains(pos): return "LT"
        if QRectF(r.right()-m, r.top()-m, m*2, m*2).contains(pos): return "RT"
        if QRectF(r.left()-m, r.bottom()-m, m*2, m*2).contains(pos): return "LB"
        if QRectF(r.right()-m, r.bottom()-m, m*2, m*2).contains(pos): return "RB"
        # Bordes
        if QRectF(r.left()+m, r.top()-m, r.width()-m*2, m*2).contains(pos): return "T"
        if QRectF(r.left()+m, r.bottom()-m, r.width()-m*2, m*2).contains(pos): return "B"
        if QRectF(r.left()-m, r.top()+m, m*2, r.height()-m*2).contains(pos): return "L"
        if QRectF(r.right()-m, r.top()+m, m*2, r.height()-m*2).contains(pos): return "R"
        # Centro
        if r.adjusted(m, m, -m, -m).contains(pos): return "CENTER"
        return None

    def hoverMoveEvent(self, event):
        zone = self._get_hotzone(event.pos())
        if zone in ("LT", "RB"): self.setCursor(Qt.CursorShape.SizeFDiagCursor)
        elif zone in ("RT", "LB"): self.setCursor(Qt.CursorShape.SizeBDiagCursor)
        elif zone in ("L", "R"): self.setCursor(Qt.CursorShape.SizeHorCursor)
        elif zone in ("T", "B"): self.setCursor(Qt.CursorShape.SizeVerCursor)
        elif zone == "CENTER": self.setCursor(Qt.CursorShape.SizeAllCursor)
    ...
    def mousePressEvent(self, event):
        ...
        if zone:
            self.drag_mode = zone
            self.drag_offset = event.pos() - self.crop_rect.topLeft() if zone == "CENTER" else None
            event.accept()
        else:
            self.drag_mode = "NEW"
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.drag_mode == "CENTER" and self.drag_offset:
            ...
            self.crop_rect = QRectF(self.drag_origin, pos).normalized().intersected(self.scene_rect)
        else:
            # Redimensionar
            l, t, right, b = r.left(), r.top(), r.right(), r.bottom()
            if "L" in self.drag_mode: l = min(pos.x(), right - self.min_size)
            if "R" in self.drag_mode: right = max(pos.x(), l + self.min_size)
            if "T" in self.drag_mode: t = min(pos.y(), b - self.min_size)
            if "B" in self.drag_mode: b = max(pos.y(), t + self.min_size)
            self.crop_rect = QRectF(QPointF(l, t), QPointF(right, b)).intersected(self.scene_rect)
