...
class LogcatFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if self.use_colors:
            ...
            level_colors = {
                logging.DEBUG: Colors.GRAY, logging.INFO: Colors.GREEN,
        ...
        if "\n" in message:
            indent_len = len(ANSI_REGEX.sub('', header)) if self.use_colors else len(header)
    ...
    if not logger.handlers:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"No se pudo habilitar faulthandler para fallos físicos: {str(fe)}")
                
        except Exception as e:
            logger.warning(f"No se pudo inicializar el archivo de logs en disco: {str(e)}")
...
def log_action(arg):
    ...
    def actual_decorator(func, msg=None):
        ...
        def wrapper(*args, **kwargs):
            classname = args[0].__class__.__name__ if args and hasattr(args[0], '__class__') else "Global"
            start_time = time.perf_counter()
            display_msg = msg if msg else f"Invoke {func.__name__}"
            
            logger.info(f"{Colors.BOLD}{display_msg}{Colors.RESET}", extra={'classname': classname})
            ...
            try:
                ...
                if duration > 1000:
                    logger.warning(f"⚠️ {Colors.YELLOW}SLOW TASK DETECTED{Colors.RESET}: {func.__name__} took {duration/1000:.2f}s", 
                                   extra={'classname': classname})
                
                logger.debug(f"└─ status: {Colors.GREEN}OK{Colors.RESET} | latency: {duration:.2f}ms", 
                             extra={'classname': classname})
                return result
            except Exception as e:
                # Al pasar exc_info=True, guardamos la línea exacta del error en el log
                logger.error(f"└─ {Colors.BOLD}CRASH DETECTED IN WORKER{Colors.RESET} -> Tipo: {type(e).__name__}", 
                             exc_info=True, extra={'classname': classname})
                raise e
        return wrapper

    if callable(arg): return actual_decorator(arg)
...
def _check_gpu():
    try:
        ...
        if 'DmlExecutionProvider' in providers:
            logger.info(f"{Colors.GREEN}--- HARDWARE IA: GPU DETECTADA (DirectML — AMD/NVIDIA/Intel) ---{Colors.RESET}")
        elif 'CUDAExecutionProvider' in providers:
            logger.info(f"{Colors.GREEN}--- HARDWARE IA: GPU DETECTADA (CUDA — NVIDIA) ---{Colors.RESET}")
        else:
            logger.warning(f"{Colors.YELLOW}--- HARDWARE IA: GPU no disponible via ONNX (Usando CPU) ---{Colors.RESET}")
        logger.info(f"Providers ONNX disponibles: {providers}")
    except ImportError:
        logger.warning(f"{Colors.YELLOW}--- HARDWARE IA: onnxruntime no encontrado ---{Colors.RESET}")
